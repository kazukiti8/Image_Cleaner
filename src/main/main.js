...
let mainWindow;
let imageAnalyzer;
let fileWatcher = null; // ファイル監視インスタンス
let watchedFolders = new Set(); // 監視中のフォルダ
...
// ファイル監視機能
function startFileWatching(folderPath) {
  try {
    safeConsoleLog('ファイル監視を開始:', folderPath);
    ...
    fileWatcher.on('add', (filePath) => {
      if (isImageFile(filePath)) {
        safeConsoleLog('画像ファイルが追加されました:', filePath);
        notifyFileChange('added', filePath);
      }
    });
    ...
    fileWatcher.on('change', (filePath) => {
      if (isImageFile(filePath)) {
        safeConsoleLog('画像ファイルが変更されました:', filePath);
        notifyFileChange('changed', filePath);
      }
    });
    ...
    fileWatcher.on('unlink', (filePath) => {
      if (isImageFile(filePath)) {
        safeConsoleLog('画像ファイルが削除されました:', filePath);
        notifyFileChange('deleted', filePath);
      }
    });
    
    // エラーハンドリング
    fileWatcher.on('error', (error) => {
      safeConsoleError('ファイル監視エラー:', error);
    });
    
    watchedFolders.add(folderPath);
    safeConsoleLog('ファイル監視が開始されました');
...
function notifyFileChange(changeType, filePath) {
  if (mainWindow && !mainWindow.isDestroyed()) {
...
// ファイル監視関連のIPCハンドラー
ipcMain.handle('start-file-watching', async (event, folderPath) => {
  try {
...
ipcMain.handle('get-watched-folders', async (event) => {
  try {
    return { success: true, folders: getWatchedFolders() };
  } catch (error) {
    safeConsoleError('監視フォルダ取得エラー:', error);
    return { success: false, error: error.message };
  }
}); 